...
class _Resource {
  ...
  constructor(uri: Uri) {
    ...
    if (UResource.isRemote(uri)) {
      const query = querystring.parse(this._uri.query) as { [x: string]: string | string[] };
      this._remoteId = parseInt(Array.isArray(query.remoteId) ? query.remoteId[0] : query.remoteId, 10);

      if (query.fsPath === undefined) {
        throw new Error(`fsPath is missing in remote uri ${this._uri}.`);
      }

      this._fsPath = Array.isArray(query.fsPath) ? query.fsPath[0] : query.fsPath;
    } else {
